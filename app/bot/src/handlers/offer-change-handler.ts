 ...
 * @param docChanges
 */
export function offerChangeHandler(client: Client, offers: Offer[], docChanges: DocumentChange<Offer>[]) {
  ...
  docChanges.forEach(async (docChange, index) => {
    ...
    if (docChange.type === 'added' && isNil(offer.postedAt)) {
      try {
        const discordGuild = offer.senderItems[0]!.collection.discordGuild
        ...
        if (!userIsInGuild(sender, discordGuild) || !userIsInGuild(receiver, discordGuild)) {
          ...
          try {
            ...
            logger.error(
              `offerChangeHandler Error adding member to thread ${thread.id} for offer ${offer.id}: ${errorMessage(e)}`
            )
          }
          try {
            await thread.send({
