...
async function removeNftsForWalletForPage<T extends Wallet>(page: number, wallet: T) {
  ...
  await PromisePool.withConcurrency(PROMISE_POOL_CONCURRENCY)
    ...
    .process(async (nft) => {
      try {
        ...
        error(`error deleting NFT ${nft.collection.slug} #${nft.tokenId}: ${errorMessage(e)}`)
      }
    })
  if (hasNext) {
    return removeNftsForWalletForPage(inc(page), wallet)
  }
}

export function removeNftsForWallet<T extends Wallet>(wallet: T) {
  return removeNftsForWalletForPage(0, wallet)
}
