...
export async function onUserUpdatedTriggerHandler(data: FirestoreEventChangeSnapshotDataReturn<UserDocument>) {
  info({ after: data.after, before: data.before }, 'user was updated')
  const {
    after: wallet,
    before,
    updated
  } = changeSnapshotDataPropUpdated<UserDocument, 'wallet'>(data, 'wallet', eqAddress)
  if (updated && !isNil(wallet)) {
    info({ user: data.after, after: wallet, before }, 'wallet changed')
    ...
    if (!isNil(wallet)) {
      info({ user: data.after, after: wallet, before }, 'wallet was removed, TODO later')
    } else {
      info({ user: data.after, after: wallet, before }, 'wallet did not change, nothing to do')
    }
  }
}
