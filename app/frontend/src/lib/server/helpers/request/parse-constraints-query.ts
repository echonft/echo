...
import { guarded_assertQueryConsraints } from '@echo/frontend/lib/server/helpers/request/assert/guarded_assert-query-consraints'
...
export function parseConstraintsQuery<T>(request: ApiRequest<T>) {
  ...
  return pipe(
    ...
    guarded_addParamFromRequest('limitToLast', positiveIntegerString),
    guarded_addParamFromRequest('offset', positiveIntegerString),
    when(propIsEmpty('params'), modify('params', always(undefined))),
    prop('params'),
    guarded_assertQueryConsraints
  )({
    request,
    params: {} as QueryConstraints
  })
}
