...
export async function searchUsersRequestHandler({ req, logger }: RequestHandlerArgs) {
  ...
  const results = await guardAsyncFn({
    fn: pipe(getUsersSearchData, andThen(pipe(filter(propSatisfies(search, 'username')), map(mapUserToSearchResult)))),
