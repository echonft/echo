...
describe('request-handlers - offer - acceptOfferRequestHandler', () => {
  ...
  it('throws if no signature is sent', async () => {
    ...
    await expect(() => acceptOfferRequestHandler({ user, req, params: { slug } })).rejects.toHaveProperty('status', 400)
  })
  ...
  it('throws if the user is not the offer receiver', async () => {
    ...
    const req = mockRequest<never>()
    await expect(() => acceptOfferRequestHandler({ user, req, params: { slug } })).rejects.toHaveProperty('status', 403)
  })
  ...
  it('returns a 200', async () => {
    ...
    const req = mockRequest<never>()
    const res = await acceptOfferRequestHandler({ user, req, params: { slug } })
