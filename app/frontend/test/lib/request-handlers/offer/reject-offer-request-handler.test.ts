...
describe('request-handlers - offer - rejectOfferRequestHandler', () => {
  ...
  it('throws if the offer does not exist', async () => {
    ...
    await expect(() => rejectOfferRequestHandler(user, req, { slug })).rejects.toHaveProperty('status', 400)
  })
  ...
  it('throws if the user is not the offer receiver', async () => {
    ...
    const req = mockRequest<never>()
    await expect(() => rejectOfferRequestHandler(user, req, { slug })).rejects.toHaveProperty('status', 403)
  })
