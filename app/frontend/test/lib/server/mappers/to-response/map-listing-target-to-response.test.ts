...
describe('mappers - to-response - mapListingTargetToResponse', () => {
  const target = getListingMockById('jUzMtPGKM62mMhEcmbN4').targets[0]
  const response: ListingTargetResponse = {
    amount: 3,
    collection: {
  ...
  it('converts the object', () => {
    expect(mapListingTargetToResponse(target)).toStrictEqual(response)
  })
})
