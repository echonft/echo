...
describe('CRUD - collection - findCollectionByAddress', () => {
  ...
  it('returns undefined if the collection if the address is wrong', async () => {
    const collection = await findCollectionByAddress(toLower('0xf672715f2bA85794659a7150e8C21F8d157bFe1D'), 1)
    expect(collection).toBeUndefined()
  })

  it('returns undefined if the collection if the chain id is wrong', async () => {
    const collection = await findCollectionByAddress(toLower('0x12c63bbD266dB84e117356e664f3604055166CEc'), 0)
    expect(collection).toBeUndefined()
  })

  it('returns the collection with the given contract address and chain id', async () => {
    const collection = await findCollectionByAddress(toLower('0x12c63bbD266dB84e117356e664f3604055166CEc'), 1)
