...
export function addSenderItemsNftCollectionIds(modelObject: PartialOffer): PartialOffer {
  return whenHas<Key, PartialOffer, OfferItem[], PartialOffer>(
    ...
    converge<
      ...
      [
        ...
        (model: PartialOfferWithSenderItems) => string[],
        (model: PartialOfferWithSenderItems) => PartialOfferWithSenderItems
      ]
    >(assoc, [
      always('senderItemsNftCollectionIds'),
      pipe(prop(key), map<OfferItem, string>(nonNullableReturn(path(['nft', 'collection', 'id']))), uniq),
      identity
    ])
  )(modelObject)
}
