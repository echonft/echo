...
export function addReceiverItemsNftIds(modelObject: PartialOffer): PartialOffer {
  return whenHas<Key, PartialOffer, OfferItem[], PartialOffer>(
    ...
    converge<
      ...
      [
        ...
        (model: PartialOfferWithReceiverItems) => string[],
        (model: PartialOfferWithReceiverItems) => PartialOffer
      ]
    >(assoc, [
      always('receiverItemsNftIds'),
      pipe(prop(key), map<OfferItem, string>(nonNullableReturn(path(['nft', 'id']))), uniq, sort(stringComparator)),
      identity
    ])
  )(modelObject)
}
