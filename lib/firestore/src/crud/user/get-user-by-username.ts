...
export function getUserSnapshotByUsername(username: Username): Promise<Nullable<QueryDocumentSnapshot<UserDocument>>> {
  return pipe(usersCollection, queryWhere('username', '==', username), getQueryUniqueDocumentSnapshot)()
}

export function getUserByUsername(username: Username): Promise<Nullable<UserDocument>> {
