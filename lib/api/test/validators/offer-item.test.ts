import { offerItem } from '../../src/types/validators/offer-item'
import { describe, expect, it } from '@jest/globals'

describe('validators - offerItem', () => {
  it('wrong address fails validation', () => {
    expect(() => offerItem.parse({ target: { address: undefined, chainId: 1 }, tokenId: 0 })).toThrow()
    expect(() => offerItem.parse({ target: { address: '', chainId: 1 }, tokenId: 0 })).toThrow()
    expect(() => offerItem.parse({ target: { address: '0xtest', chainId: 1 }, tokenId: 0 })).toThrow()
    expect(() =>
      offerItem.parse({
