...
import { getChainsForVirtualMachine } from '@echo/model/helpers/chain/get-chains-for-virtual-machine'
...
async function updateNftsForContract(contract: Contract): Promise<void> {
  const nftGroups = await fetchNftsByAccount(contract)
...
export async function updateNftsForWallet(wallet: Wallet): Promise<void> {
  ...
  const chains = pipe(prop('vm'), getChainsForVirtualMachine)(wallet)
  for (const chain of chains) {
    await updateNftsForContract({ address: wallet.address, chain })
  }
