...
import { getNft, getNftSnapshot } from '@echo/firestore/crud/nft/get-nft'
import { getNftsForWalletAndCollection } from '@echo/firestore/crud/nft/get-nfts-for-wallet-and-collection'
...
import { andThen, assoc, collectBy, equals, head, isNil, map, path, pick, pipe, prop } from 'ramda'
    ...
    for (const nftGroup of groupedNfts) {
      ...
      for (const nft of nftGroupWithCollection) {
        try {
          ...
          if (isNil(existingNft)) {
            logger?.info({ nft, wallet }, 'NFT is not in the database')
            try {
              if (isNil(user)) {
                ...
                logger?.info({ nft }, 'added NFT to the database')
              }
            } catch (err) {
              logger?.error({ err, nft }, 'could not add NFT to the database')
            }
          } else if (!equals(existingNft.owner.wallet, wallet)) {
            logger?.info({ nft, wallet }, 'NFT ownership changed')
            try {
              if (isNil(user)) {
                logger?.error({ nft, wallet }, 'cannot add NFT because no owner found for the wallet')
              } else {
      ...
      for (const ownedNft of ownedNfts) {
        if (!includesWith(ownedNft, eqNft, nftGroupWithCollection as Nft[])) {
          try {
            logger?.info({ nft: ownedNft, wallet }, 'NFT is in the database, but actual owner is not')
            ...
            if (isNil(snapshot)) {
              ...
              logger?.info({ nft: ownedNft }, 'deleted NFT from database')
            }
          } catch (err) {
