...
import { createOfferQueryParamsSchema } from '@echo/routing/validators/frontend/offer/create-offer-query-params-schema'
...
describe('createOfferQueryParamsSchema', () => {
  function expectZodError(data: unknown, path: (string | number | undefined)[]) {
    ...
    try {
      createOfferQueryParamsSchema.parse(data)
    } catch (err) {
  ...
  test('valid without target', () => {
    expect(createOfferQueryParamsSchema.parse({ items: [nftMockPx1, nftMockPx2] })).toStrictEqual({
  ...
  test('valid with target', () => {
    expect(
      createOfferQueryParamsSchema.parse({ items: [nftMockPx1, nftMockPx2], target: collectionMockPx })
    ).toStrictEqual({
      items: [nftIndex(nftMockPx1), nftIndex(nftMockPx2)],
