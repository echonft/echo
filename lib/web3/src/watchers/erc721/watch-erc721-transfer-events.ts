...
interface WatchErc721TransferEventHandlerArgs<T extends ChainName> {
  log: Erc721TransferEventLog
  chain: T
}

interface WatchErc721TransferEventsArgs<T extends ChainName> {
  client: ViemClient<T>
...
export function watchErc721TransferEvents<T extends ChainName>(args: WatchErc721TransferEventsArgs<T>) {
  ...
  return client.watchContractEvent({
    ...
    onError: (error: Error) => {
      console.error(`error parsing 'Transfer' event log on ERC721: ${errorMessage(error)}`)
    }
  })
}
