...
export const OfferDetailsAcceptModal: FunctionComponent<Props> = ({
  ...
  return (
    // FIXME DEV-156
    <Modal open={open} onClose={onClose} title={t('title')} closeDisabled={isAccepting}>
      <div className={clsx('flex', 'flex-col', 'gap-6', 'items-center', 'self-stretch')}>
        ...
        <OfferDetailsAcceptModalButtons
          ...
          token={token}
          chainId={chain?.id}
          acceptOfferFetcher={(offerId, signature, token) => {
            setIsAccepting(true)
            return acceptOfferFetcher(offerId, signature, token)
          }}
          onSuccess={onSuccess}
          onError={onError}
        />
