...
export const OfferDetailsModalBody: FunctionComponent<OfferDetailsModalProps> = ({ offer, onUpdate, onClose }) => {
  if (isNil(offer)) {
    return null
  }
  return (
    <div className={clsx('w-[66vw]', 'h-max', 'max-w-[70rem]', 'p-4', 'px-8')}>
      <OfferDetails offer={offer} onUpdate={onUpdate} onClose={onClose} />
    </div>
  )
}
