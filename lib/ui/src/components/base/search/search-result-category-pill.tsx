...
export const SearchResultCategoryPill: FunctionComponent<Props> = ({
  ...
  return (
    ...
    >
      <span className={clsx('prose-label-md', 'text-white')}>{t(category, { count })}</span>
      <div className={clsx('w-6.5', 'h-6.5', 'rounded-lg', 'flex', 'items-center', 'justify-center', 'bg-dark-300')}>
        <span className={clsx('prose-other-medium', 'text-white/50')}>{count}</span>
      </div>
    </button>
  )
}
