...
describe('validators - collectionResponseSchema', () => {
  ...
  const response = {
    ...
    contracts: [mock.contract, assoc('chain', 'blast', mock.contract)],
  ...
  it('with chain check', () => {
    expect(collectionResponseSchema({ chain: 'blast' }).parse(response)).toStrictEqual(
      assocPath(['contract', 'chain'], 'blast', mock)
    )
    expect(collectionResponseSchema({ chain: 'blast_sepolia' }).parse(response)).toBeUndefined()
  })
})
